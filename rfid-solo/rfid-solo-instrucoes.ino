...
void loop() {
  ...
  //SE tiver um novo cartao proximo no leitor do RDId (essa funcao esta dentro da biblioteca...acessando os 2 pinos especificados inicialmente), ENTAO vai para a proxima condicional IF (SE)
  if ( ! mfrc522.PICC_IsNewCardPresent()) {
    return;
  }

  // Select one of the cards
  //SE ele achou um cartao, então vai ler esse cartao
  ...
  //ele vai percorrer todo o mapa da EPROM (memoria)
  ...
  //só pra transformar a tradução em caracteres em caixa alta (CAPS)
  content.toUpperCase();

  //change here the UID of the card/cards that you want to give access //SE essa tradução for igual a essa string/frase "71 4A 2C 2B" , ele vai escrever no monitor serial (Acesso autorizado)
  if (content.substring(1) == "71 4A 2C 2B") {
    Serial.println("Authorized access");
    Serial.println();
    delay(3000); //Ele vai esperar 3 segundos para fazer o novo LOOP
  } else {
    //SENAO (caso nao seja aquela frase/string , ele vai escrever no monitor serial (Acesso negado)
